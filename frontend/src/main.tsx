...
function App() {
  ...
  async function openIncident(id: string) {
    ...
    setLoading(true);
    try {
      const [loadedIncident, loadedTimeline, loadedEvidence, loadedDashboard, loadedPlans] = await Promise.all([
      ...
      setReport(null); setConversation([]); setPlan(latestPlan); setRecovery(latestPlan?.recovery ?? null); setPostmortem(null); setMessage("");
  ...
  async function refreshQueue(preferredId?: string, autoOpen = false) {
    try {
      ...
      setQueue(items);
      if (autoOpen) {
        const selected = preferredId ? items.find((item) => item.id === preferredId) : undefined;
        if (selected) await openIncident(selected.id);
        else if (preferredId) setMessage("The requested incident is not open. Select a controlled incident below or start a new rehearsal.");
      }
    } catch {
  ...
  return <main className="app-shell">
    ...
    {!incident && <section className="empty-state"><p className="eyebrow">Controlled incident command center</p><h1>Select an incident before triage</h1><p>This page does not show a general system dashboard. Start a P1/P2 rehearsal or open one specific persisted incident record.</p><div className="source-boundary"><strong>What each selected incident can read</strong><span><b>Prometheus:</b> live checkout route rates and trend</span><span><b>Kubernetes:</b> checkout workload status, events, and deployment revisions</span><span><b>Local incident store:</b> scenario alert, evidence trail, approvals, and audit events</span><span><b>GitHub:</b> not connected in this controlled demo</span></div>{queue.length ? <section className="empty-queue" aria-label="Open controlled incidents"><p className="eyebrow">Open controlled incidents</p><p>These are persisted local simulation records. Select one only if you intend to review that incident trail.</p><ul className="queue-bullets">{queue.slice(0, 5).map((item) => <li key={item.id}><button type="button" aria-label={`Open ${item.alert_name} incident ${item.id.slice(-6)}`} onClick={() => void openIncident(item.id)}><span className={`queue-severity severity-${item.severity.toLowerCase()}`}/><span><strong>{item.alert_name}</strong><small>{item.service} · {displayState(item.lifecycle_state)} · {incidentAge(item.created_at)} · #{item.id.slice(-6)}</small></span></button></li>)}</ul></section> : <p className="muted">No open controlled incidents. Use Inject P1 incident or select P2 to create one.</p>}</section>}
      ...
      <section className="command-layout">
        ...
        <section className="main-pane">
          ...
          <section className="panel investigation-panel"><p className="eyebrow">Conversational investigation</p><h2>Ask a follow-up question</h2><p className="muted">Each question takes a fresh Prometheus and Kubernetes evidence snapshot. The model can cite it, but can never execute a remediation.</p>{dashboard.investigation_mode === "controlled_simulation" ? <p className="simulation-notice">Controlled rehearsal reports are enabled. They are deterministic evidence summaries, not GPT-5.6 output.</p> : <p className="muted">Live-model mode is active. If API access is unavailable, restart the local API with <code>run-console.ps1 -ControlledSimulation</code> for an explicitly labelled rehearsal.</p>}<div className="question-chips"><button type="button" onClick={() => setQuestion("What changed before the alert?")}>What changed?</button><button type="button" onClick={() => setQuestion("What evidence supports the current root-cause hypothesis?")}>Show evidence</button><button type="button" onClick={() => setQuestion("What is the confirmed affected scope?")}>What is affected?</button><button type="button" onClick={() => setQuestion("What are the current 5xx rate, request rate, readiness, and restarts?")}>Current health</button></div><div className="chat-compose"><input value={question} onChange={(event) => setQuestion(event.target.value)} aria-label="Ask about the incident"/><button disabled={loading} type="button" onClick={investigate}>{loading ? "Collecting evidence…" : "Investigate"}</button></div>
            {conversation.length ? conversation.map((turn, turnIndex) => <div className="conversation-turn" key={`${turn.question}-${turnIndex}`}><div className="chat-message engineer"><span>ON</span><div><small>On-call engineer</small><p>{turn.question}</p></div></div><ReportMessage report={turn.report}/></div>) : <div className="empty-investigation">{dashboard.investigation_mode === "controlled_simulation" ? "Controlled rehearsal reports are enabled. They are deterministic evidence summaries, not GPT-5.6 outputs." : "No model report recorded. Configure the test provider to run a live GPT-5.6 investigation; no hypothesis or confidence is fabricated here."}</div>}
          </section>
